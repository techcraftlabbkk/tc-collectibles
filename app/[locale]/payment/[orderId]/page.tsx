  ...
  if (!order) {
    return (
      <div className="min-h-screen bg-white flex items-center justify-center flex-col gap-4">
        ...
        <Link href={`/${locale}/products`} className="text-emerald-600 hover:underline">Back to Products</Link>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="w-full px-4 py-10 bg-gradient-to-r from-emerald-600 to-teal-600 text-white">
        <div className="max-w-2xl mx-auto">
          ...
          <p className="text-emerald-200 mt-1 text-sm">Order #{params.orderId.slice(0, 8).toUpperCase()}</p>
        </div>
      </section>
      ...
      <div className="max-w-2xl mx-auto px-4 py-10 space-y-6">
        ...
        <div className="bg-gradient-to-br from-emerald-50 to-teal-50 border-2 border-emerald-200 rounded-2xl p-6">
          ...
          <div className="flex justify-between items-baseline border-t-2 border-emerald-200 pt-4">
            <span className="font-black text-gray-900 text-lg">Total</span>
            <span className="text-4xl font-black text-emerald-600">{'฿'}{order.total.toLocaleString()}</span>
          </div>
        </div>
        ...
        <div className="bg-white border-2 border-emerald-100 rounded-2xl overflow-hidden">
          <div className="grid grid-cols-2 border-b-2 border-emerald-100">
            <button
              onClick={() => setMethod('promptpay')}
              className={`py-4 font-bold text-sm transition-colors ${method === 'promptpay' ? 'bg-emerald-600 text-white' : 'text-gray-600 hover:bg-emerald-50'}`}
            >
              {'📱'} PromptPay QR
            </button>
            <button
              onClick={() => setMethod('card')}
              className={`py-4 font-bold text-sm transition-colors ${method === 'card' ? 'bg-emerald-600 text-white' : 'text-gray-600 hover:bg-emerald-50'}`}
            >
              {'💳'} Credit / Debit Card
            </button>
          </div>
              ...
              <div className="text-center">
                ...
                {qrLoading ? (
                  ...
                  <div className="flex justify-center">
                    <div className="bg-white p-4 rounded-2xl shadow-lg border-2 border-emerald-100 inline-block">
                      <Image src={qrCode} alt="PromptPay QR" width={220} height={220} />
                    </div>
                  </div>
                ) : (
                  <button onClick={generateQR} className="bg-emerald-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-emerald-700 transition-colors">
                    Generate QR Code
                  </button>
                )}
