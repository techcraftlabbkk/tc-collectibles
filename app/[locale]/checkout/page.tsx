...
export default function Checkout() {
  ...
  const inp = 'w-full px-4 py-3 border-2 border-purple-200 rounded-xl focus:outline-none focus:border-purple-600 focus:ring-2 focus:ring-purple-100 font-medium text-gray-900 placeholder-gray-400 transition';
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="w-full px-4 py-10 bg-gradient-to-r from-purple-600 to-indigo-600 text-white">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-base text-purple-200 mt-1">Secure checkout · PromptPay payment</p>
        </div>
      </section>
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
              ...
              <div key={s.id} className="flex items-center gap-2">
                <div className={`flex items-center gap-2 ${active ? 'opacity-100' : done ? 'opacity-80' : 'opacity-40'}`}>
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all ${
                    done ? 'bg-green-500 text-white' : active ? 'bg-purple-600 text-white ring-4 ring-purple-100' : 'bg-gray-200 text-gray-500'
                  }`}>
                    {done ? '✓' : i + 1}
                  </div>
                  <span className={`text-xs font-bold ${active ? 'text-purple-700' : 'text-gray-500'}`}>{s.label}</span>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {step === 'review' && (
              <div className="space-y-5">
                ...
                <div className="bg-white border-2 border-purple-100 rounded-2xl p-6">
                  <div className="flex items-center justify-between pb-4 border-b-2 border-purple-50 mb-4">
                    <h2 className="text-base font-black text-gray-900">Your Items ({items.length})</h2>
                    <button onClick={() => setStep('address')} className="text-xs text-purple-600 font-bold hover:underline">← Edit address</button>
                  </div>
                  <div className="space-y-3">
                    {items.map(item => (
                      <div key={item.product_id} className="flex items-center gap-3 py-2 border-b border-gray-50 last:border-0">
                        ...
                        <p className="text-sm font-black text-purple-600 flex-shrink-0">฿{(item.price * item.quantity).toLocaleString()}</p>
                      </div>
                    ))}
                  </div>
                  <div className="mt-4 pt-3 border-t-2 border-purple-50 space-y-1">
                ...
                {/* Terms + Place Order */}
                <div className="bg-white border-2 border-purple-100 rounded-2xl p-6 space-y-4">
                  <label className="flex items-start gap-3 cursor-pointer">
                    ...
                    <span className="text-sm text-gray-700">
                      ...
                      <Link href={`/${locale}/terms`} className="text-purple-600 hover:underline">Terms &amp; Conditions</Link>
                    </span>
                  </label>
          ...
          <div className="lg:col-span-1">
            <div className="bg-gradient-to-br from-purple-50 to-indigo-50 border-2 border-purple-200 rounded-2xl p-6 sticky top-8">
